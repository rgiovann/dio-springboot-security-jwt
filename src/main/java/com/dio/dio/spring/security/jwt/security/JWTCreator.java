package com.dio.dio.spring.security.jwt.security;
...
//        public static String create(String prefix,String key, JWTObject jwtObject) {
//        String token = Jwts.builder().setSubject(jwtObject.getSubject()).setIssuedAt(jwtObject.getIssuedAt()).setExpiration(jwtObject.getExpiration())
//                .claim(ROLES_AUTHORITIES,  Arrays.asList(new String[]{"ROLEUSERS", "ROLEMANAGERS"})).signWith(SignatureAlgorithm.HS512, key).compact();
//        return prefix + " " + token;
//    }
    ...
    private static List<String> checkRoles(List<String> roles) {
        ...
        novaLista = roles.stream().map(s -> "ROLE_".concat(s.replaceAll("ROLE_",""))).collect(Collectors.toList());
        return novaLista;
    }
